...
import (
	...
	"github.com/Xfers/cloudhsm-service-go/controllers"
...
func main() {
	...
	//set flag for private key path
	privateKeyPath := flag.String("private_key_path", "", "Path to private key")
	// Set flag for public key path
	publicKeyPath := flag.String("public_key_path", "", "Path to public key")
	flag.Parse()

	// Run server
	r := gin.Default()

	// Digest endpoint to point to controller
	r.POST("/digest", func(c *gin.Context) {
		controllers.DigestController(c)
	})

	// Sign endpoint to point to controller
	r.POST("/sign", func(c *gin.Context) {
		//add private key to gin context
		c.Set("private_key_path", *privateKeyPath)

		controllers.SignController(c)
	})

	// Verify endpoint to point to controller
	r.POST("/verify", func(c *gin.Context) {
		//add public key to gin context
		c.Set("public_key_path", *publicKeyPath)
